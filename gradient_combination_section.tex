...
The combination of \texttt{copy} operations with custom formulas enables highly sophisticated composite methods. For instance, a focal-point approach that reuses an expensive high-level calculation multiple times within different mathematical contexts can be expressed as:
\begin{verbatim}
!scheme
! copy[1,0.5] copy[1,1.5]
!formula
! e=exp(c1*e1/100)+c2*e2
!end
\end{verbatim}
Here, calculation 1 (the expensive high-level reference) is reused twice: first with coefficient 0.5 inside a damped exponential function (providing variable \texttt{e1}), and second with coefficient 1.5 as a direct additive correction (providing variable \texttt{e2}). Critically, both \texttt{e1} and \texttt{e2} refer to the same energy $E_1$, eliminating the need to execute a second expensive calculation. The final energy expression becomes $E_{\text{final}} = \exp(0.5 E_1 / 100) + 1.5 E_1$, demonstrating the power of combining algebraic flexibility with computational efficiency through selective gradient reuse. This approach is particularly valuable when a single high-level calculation (e.g., CCSD(T) at a large basis set) must contribute to multiple terms in a composite energy expression with different functional forms or scaling factors.
